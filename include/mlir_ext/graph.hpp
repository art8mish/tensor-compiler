...
public:
    MLIRComputeGraph(std::unique_ptr<mlir::MLIRContext> ctx, mlir::OwningOpRef<mlir::ModuleOp> mod)
    ...
    const mlir::MLIRContext &context() const {
        return *context_;
    }
    ...
    const mlir::ModuleOp module() const {
        return module_.get();
    }
